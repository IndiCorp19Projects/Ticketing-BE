...
} = require("../utils/escalationEmailTemplates");
const calculateWorkingHours = require("../utils/calculateWorkingHours");

function getEscalationLevel(hours, escalationLevels) {
  if (!Array.isArray(escalationLevels) || escalationLevels.length === 0) {
    throw new Error("Invalid escalationLevels array");
  }

  // Sort levels by escalation_rule (ascending)
  const sortedLevels = [...escalationLevels].sort(
    (a, b) => Number(a.escalation_rule) - Number(b.escalation_rule)
  );

  // Find the matching level
  let selectedLevel = sortedLevels[0]; // Default to first

  for (let i = 0; i < sortedLevels.length; i++) {
    const current = Number(sortedLevels[i].escalation_rule);
    const next = Number(sortedLevels[i + 1]?.escalation_rule ?? Infinity);

    if (hours >= current && hours < next) {
      selectedLevel = sortedLevels[i];
      break;
    }
  }
...
exports.sendEscalationReminder = async (req, res) => {
  ...
  try {
    ...
    this.sendEscalationReminderEmail(
      escalation,
      req.client_user,
      reminder_message,
      cc,
 ...
 */
exports.sendEscalationReminderEmail = async (
  escalation,
  clientUser,
  reminderMessage,
  cc = null,
  ...
  try {
    ...
    console.log(
      `Escalation reminder sent to: ${escalation.escalated_to_email}`
    );
    if (mailOptions.cc) console.log(`CC: ${mailOptions.cc.join(", ")}`);
    if (mailOptions.bcc) console.log(`BCC: ${mailOptions.bcc.join(", ")}`);
  } catch (error) {
