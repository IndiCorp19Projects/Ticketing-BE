...
//     if (obj.dataValues && Object.prototype.hasOwnProperty.call(obj.dataValues, key)) return obj.dataValues[key];
//     return undefined;
//   };
...
//   // If ticket.client_id exists and does not match req.client.id -> deny
//   if (ticketClientId != null && req.client && req.client.id != null) {
//     if (String(ticketClientId) !== String(req.client.id)) {
//       console.log(`[perm-check] DENY: client id mismatch: ticket.client_id=${ticketClientId}, req.client.id=${req.client.id} (ticket ${ticketId})`);
//       return false;
//     }
//   }

//   // Admins of the client can access everything for that client
//   if (req.client_user && (req.client_user.role === 'admin' || req.client_user.client_user_role === 'admin')) {
//     return true;
//   }

//   // Collect the runtime user-id from multiple possible keys the middleware might provide
//   const runtimeUserId = req.client_user && (req.client_user.id ?? req.client_user.client_user_id ?? req.client_user.user_id ?? req.client_user.client_id ?? null);
//   if (!runtimeUserId) {
//     console.log('[perm-check] DENY: no runtime user id found on req.client_user', req.client_user);
//     return false;
//   }

//   // Ticket creator and assigned client user
//   const ticketCreatorId = get(ticket, 'client_user_id');
//   const ticketAssignedId = get(ticket, 'assigned_client_user_id');

//   const isCreator = ticketCreatorId != null && String(ticketCreatorId) === String(runtimeUserId);
//   const isAssigned = ticketAssignedId != null && String(ticketAssignedId) === String(runtimeUserId);

//   if (isCreator || isAssigned) {
//     return true;
//   }

//   console.log(`[perm-check] DENY: user (${runtimeUserId}) not creator (${ticketCreatorId}) or assignee (${ticketAssignedId}) for ticket ${ticketId}`);
//   return false;
// }
...
//   const ticketCreatorId = String(ticket.client_user_id || '');
//   const ticketAssignedUserId = String(ticket.assigned_client_user_id || '');

//   // If ticket is assigned to someone (assigned_client_user_id has value)
//   if (ticketAssignedUserId && ticketAssignedUserId !== '') {
//     // Only the assigned user can access the ticket
//     return ticketAssignedUserId === currentUserId;
//   } else {
//     // If ticket is not assigned, creator can access it
...
//     // UPDATED: Check permissions - only assigned user and admin can access
...
//     // UPDATED: User permissions based on new rules
//     const userPermissions = {
//       can_reply: canReplyToTicket(req, ticket),
...
// UPDATED: Permission check function - only assigned user and admin can access
...
// // UPDATED: Check if user can reply to ticket old chandrashekhar 
...
//     // If client_user (role 'user'), show tickets where:
//     // - they are the creator (client_user_id) OR
//     // - ticket is assigned to them (assigned_client_user_id)
//     if (req.client_user.role === 'user') {
//       const userId = Number(req.client_user.id);
//       // Use OR inside the client scope
//       whereCondition = {
//         client_id: clientId,
//         [Op.or]: [
...
//     // Search (keep existing behavior)
//     if (search) {
//       whereCondition[Op.or] = whereCondition[Op.or] ? whereCondition[Op.or].concat([
...
//         { model: TicketReply, as: 'replies', include: [{ model: Document, as: 'documents', attributes: ['document_id', 'doc_name', 'mime_type', 'created_on'] }] },
//         { model: Document, as: 'documents', attributes: ['document_id', 'doc_name', 'mime_type', 'created_on'] },
...
//     // post-process tickets (SLA & replies) same as before...
//     const ticketsWithSLA = await Promise.all(
//       tickets.map(async (ticket) => {
//         const plain = ticket.toJSON ? ticket.toJSON() : ticket;
//         if (Array.isArray(plain.replies)) {
...
//         name: req.client_user.name,
//         role: req.client_user.role,
//         permissions: req.client_user.role === 'admin' ?
//           ['view_all_tickets', 'assign_tickets', 'escalate_tickets'] :
//           ['view_own_tickets', 'create_tickets']
//       },
//       pagination: {
//         total: count,
...
//         // If there's already an OR condition (from role filtering), merge with search
...
//       ['view_all_tickets', 'assign_tickets', 'escalate_tickets'] :
//       ['view_assigned_tickets', 'create_tickets'];
...
//     // UPDATED: For non-admin users, show tickets they created OR tickets assigned to them
//     if (req.client_user.role === 'user') {
//       const userId = Number(req.client_user.id);
//       whereCondition = {
//         client_id: clientId,
//         [Op.or]: [
//           // Tickets created by the user (regardless of assignment)
//           { client_user_id: userId },
...
//     // For non-admin users
//     if (req.client_user.role === 'user') {
//       const userId = String(req.client_user.client_user_id);

//       whereCondition = {
//         client_id: clientId,
//         [Op.or]: [
//           // Tickets where user is creator AND ticket is not assigned to someone else
//           {
//             [Op.and]: [
//               { client_user_id: userId },
//               { 
